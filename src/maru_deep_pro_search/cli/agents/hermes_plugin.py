...
def _check_research() -> tuple[bool, str]:
    ...
    if not SESSION_FILE.exists():
        return False, "No research session found. Run /ask or /research <query> first."
    ...
    if now - ts > timedelta(minutes=SESSION_TTL_MINUTES):
        return (
            False,
            f"Research expired (TTL={SESSION_TTL_MINUTES}min). Re-run /ask or /research.",
        )
...
def register(ctx) -> None:
    ...
    def on_session_start(session_id: str, **kwargs) -> None:
        ...
        ctx.inject_message(
            ...
            "Run /ask or /research <query> before using other tools.",
            role="system",
        )
