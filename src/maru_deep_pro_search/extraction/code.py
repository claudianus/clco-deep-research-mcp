...
_LANG_SIGNATURES: list[tuple[str, re.Pattern]] = [
    ...
    ("cpp", re.compile(r"\b(#include|using namespace|std::|int main\(|cout <<|cin >>)", re.MULTILINE)),
    ("csharp", re.compile(r"\b(using System|namespace |public class|private void|Console\.Write|async Task)", re.MULTILINE)),
    ("php", re.compile(r"\b(<\?php|echo |function\s+\w+\(|\$\w+|->)\b", re.MULTILINE)),
    ("ruby", re.compile(r"\b(def |class |module |require |attr_|puts |\.each\b|\.map\b)", re.MULTILINE)),
...
_PACKAGE_PATTERNS = {
    "python": re.compile(r"(?:pip install|requirements\.txt|pyproject\.toml).*?([a-zA-Z0-9_-]+(?:\[.*?\])?(?:==[\d.]+)?)", re.MULTILINE),
    "javascript": re.compile(r"(?:npm install|yarn add|package\.json).*?([a-zA-Z0-9@/_-]+(?:@[\d.]+)?)", re.MULTILINE),
    "go": re.compile(r"go get\s+([a-zA-Z0-9./_-]+)", re.MULTILINE),
    "rust": re.compile(r"(?:cargo add|Cargo\.toml).*?([a-zA-Z0-9_-]+(?:\s*=\s*\"[\d.]+\")?)", re.MULTILINE),
}
